...
public class Card implements java.io.Serializable {
	...
	public static void main(String[] args) {
		try {
			...
			if (args.length == 0) {
				...
				p = new PrintWriter(new FileWriter(args[0]));
	...
	public static Card read(BufferedReader in) throws EOFException {
		...
		try {
			...
			for (int i = 0; i < SUIT_STRINGS.length; i++) {
				if (split[0].equals(SUIT_STRINGS[i])) {
					ss = suitString2Char(split[0]);
				}
			}
			for (int i = 0; i < RANK_STRINGS.length; i++) {
				if (split[1].equals(RANK_STRINGS[i])) {
					sr = rankString2Char(split[1]);
				}
			}
			card = new Card(ss, sr);
	...
	public static Card read(DataInput in) {
		...
		char sr = 'n';
		try {
			char text = in.readChar();
			char text2 = in.readChar();
			for (int i = 0; i < SUIT_CHARACTERS.length; i++) {
				if (text == (SUIT_CHARACTERS[i])) {
					ss = text;
				}
			}
			for (int i = 0; i < RANK_CHARACTERS.length; i++) {
				if (text2 == (RANK_CHARACTERS[i])) {
					sr = text2;
				}
			}
			card = new Card(ss, sr);
